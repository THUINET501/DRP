...
class BaseRadon(abc.ABC):
    ...
    def filter_sinogram(self, sinogram, filter_name="ramp"):
        ...
        pad1 = int((padded_size - size)/2)
        pad2 = padded_size - size - pad1
        padded_sinogram = F.pad(sinogram.float(), (pad1, pad2, 0, 0))
        ...
        sino_fft = torch.view_as_real(torch.fft.fft(padded_sinogram,  dim = -1))
        ...
        filtered_sinogram = torch.fft.ifft(torch.view_as_complex(filtered_sino_fft),  dim= -1)
        
        # pad removal and rescaling
        filtered_sinogram = filtered_sinogram[:, :, pad1:pad1+size] * (np.pi / (2 * n_angles))
